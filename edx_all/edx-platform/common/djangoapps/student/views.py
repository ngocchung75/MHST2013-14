...
from courseware.courses import get_courses, sort_by_announcement,get_courses_id
...
#hung add
def search_courses(str,uni, course_id, course_name):
    from django.db import  connection
    cursor=connection.cursor()
    q="""SELECT courseware_studentmodule.course_id, courseware_studentmodule.id FROM courseware_studentmodule
    WHERE course_id LIKE %s
    ORDER BY courseware_studentmodule.id DESC
    """
    value=''
    if uni !=0:
        value='{}%'.format(str)
    ...
    data_list=dict(cursor.fetchall())
...
def search(request):
    ...
    if request.method=='GET':
        ...
        name=request.GET.get('courseName','')
    ...
    if school!='':
        ...
        courses = get_courses_id(None,school,uni=1, domain=domain)
    elif subject!='':

        courses=get_courses_id(None,subject,course=1, domain=domain)

    elif name!='':

        courses=get_courses_id(None,name,courseName=1, domain=domain)
    else:
...
def create_account(request, post_override=None):
    ...
    try:
	    ...
	    subj='Xin Bạn hãy hoàn thành việc kích hoạt tài khoản'
            site='edx.dtu.vn:8001'
	    site1='edx.dtu.vn:8000'
            key=registration.activation_key
            content="""
            Cảm ơn bạn đã đăng ký tài khoản ở Học Viên Mở Việt Nam.Để kích hoạt tài khoản, xin vui lòng click vào các link này:
            <br></br>
            <br></br>
            <br></br>
	    Kích hoạt cho hệ thống cho giảng viên:
	    <br></br>
            http://{site}/activate/{key}
	    <br></br>
	    Kích hoạt cho hệ thống người học:
	    <br></br>
            http://{site1}/activate/{key}
            <br></br>
            <br></br>
            <br></br>
            Nếu bạn không có yêu cầu này, bạn không cần làm bất kỳ điều gì.Và bạn cũng không nhân được bất kỳ điều gì từ chúng tôi.
            """
            content=content.format(site=site,site1=site1,key=key)
            send_email_edx('hoangthehung92@hocvienmovietnam.mygbiz.com','thehung1992','hoangthehung92@gmail.com',user.email,subj,content)
...
#hung add
def send_email_edx(email_server=None,pass_server=None,user_server=None, to_user=None,  subj=None,content=None):
    msg = MIMEText(content, 'html','utf-8')
    msg['Subject'] =subj
    msg['From'] =user_server
    msg['To'] = to_user
    connection=False
    ...
    server.starttls()
    server.ehlo()
    server.login(email_server,pass_server)
    server.sendmail(user_server,to_user,msg.as_string())
    server.quit()
    # server.close()
...
def password_reset(request):
    ...
    content="""
            Bạn Nhận được email này vì bạn đã yêu cầu thiết lập lại tài khoản người dùng tại Học Viên Mỡ Việt Nam.
            <br></br>
            Hãy truy cập vào trang sau và chọn một mật khẩu mới:
            <br></br>
            <br></br>
            http://{site}/password_reset_confirm/{key}
            <br></br>
            <br></br>
            <br></br>
            Nếu bạn không có yêu cầu thay đổi mật khẩu, bạn không nên click nào liên kết này. Và chúng tôi sẽ không thay đổi mật khẩu của bạn.
            """
    subj='Thay đổi mật khẩu'
    site='edx.dtu.vn:8000'
    ...
    if form.is_valid():
        ...
        id_change='{uid}-{token}'.format(uid=uid,token=token)
        content=content.format(site=site,key=id_change)
        # send_email_edx(email_server=None,pass_server=None,user_server=None, to_user=None,  subj=None,content=None):

        send_email_edx('hoangthehung92@hocvienmovietnam.mygbiz.com','thehung1992','hoangthehung92@gmail.com',request.user.email,subj,content)
        return HttpResponse(json.dumps({'success': True,
...
def change_email_request(request):
    ...
    content="""
        Chúng tôi nhân được yêu cầu thay đổi email liên kết của bạn với tài khoản ở Học Viên Mỡ Việt Nam <br></br> từ {email1} đến {email2}.
        <br></br>
        <br></br>
        Nếu điều đó là chính xác, xin bạn hãy xác nhân email mới bằng đường link dưới đây:
        <br></br>
        <br></br>
        ...
        <br></br>
        <br></br>
        Nếu bạn không có yêu cầu này, bạn không nên click nào liên kết này. Và chúng tôi sẽ không thay đổi email của bạn của bạn.
        """
    subj='Thay đổi email'
    site='edx.dtu.vn:8000'
    content=content.format(email1=user.email,email2=pec.new_email,site=site,key=pec.activation_key)
    send_email_edx('hoangthehung92@hocvienmovietnam.mygbiz.com','thehung1992','hoangthehung92@gmail.com',pec.new_email,subj,content)
